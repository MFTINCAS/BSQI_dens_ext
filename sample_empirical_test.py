...
from distr_emp_bs_1 import BSemp
import numpy as np
...
NIT = 2
printfig = True
meth_bins = 'rice' #'scott','rice', 'fd', 'knuth_bin'
...
data = [
    ...
    ["BSHQI", "{:.2e}".format(AMISEqi_TOT), "{:.2e}".format(MSEqi_TOT),
     "{:.2e}".format(test_1_bs[0]), "{:.2e}".format(test_1_bs[1]),
     "{:.2e}".format(test_2_bs.statistic), "{:.2e}".format(test_2_bs.pvalue)],
    ["\hline"],
    ["EMPIRICAL", "{:.2e}".format(AMISE_TOT), "{:.2e}".format(MSE_TOT), "{:.2e}".format(test_1[0]),
     "{:.2e}".format(test_1[1]),"{:.2e}".format(test_2.statistic), "{:.2e}".format(test_2.pvalue)]
]
